    ...
    <section className="border-y border-[#e4dbcf] bg-[#f8f5f0] text-[#111827]">
      <div className="mx-auto max-w-[1440px] px-6 py-20 sm:px-8 lg:px-12 lg:py-24 xl:px-16">
        <div className="text-center">
          <p className="text-sm font-bold uppercase tracking-[0.26em] text-[#b77b13]">How Mitch Helps</p>
          <h2 className="serif mt-3 text-5xl text-[var(--aronson-navy)] sm:text-6xl">Practice Focus</h2>
          <div className="mx-auto mt-4 h-0.5 w-16 bg-[#d3a13a]" />
        </div>

        <div className="mt-14 grid grid-cols-1 gap-7 lg:grid-cols-3">
          {practices.map((practice) => (
            <article key={practice.title} className="group border border-[#e2d9cd] bg-white p-9 shadow-[0_12px_35px_rgba(7,23,38,0.06)] transition duration-300 hover:-translate-y-1 hover:border-[#d3a13a]/70 hover:shadow-[0_18px_44px_rgba(7,23,38,0.1)] sm:p-10">
              <div className="flex items-center gap-4">
                <span className="flex h-16 w-16 shrink-0 items-center justify-center rounded-full bg-[var(--aronson-garnet)] text-white">
                  <PracticeIcon type={practice.icon} />
                </span>
                <h3 className="serif text-[1.9rem] leading-[1.12] text-[var(--aronson-navy)]">{practice.title}</h3>
              </div>
              <ul className="mt-8 space-y-5">
                {practice.items.map((item) => (
                  <li key={item} className="flex gap-3 text-lg leading-8 text-[#303844]">
